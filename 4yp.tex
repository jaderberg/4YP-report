...
This project builds upon the vast amount of previous work done on large scale image retrieval.

There is no general algorithm to determine whether a query image\footnote{A query image is one that contains a depiction of an object that is to be identified.} contains the same object as an image in a dataset - the variation in viewpoint, lighting, environment and noise is so large that this task can even be a struggle for humans. If one did exist, applying this algorithm to every image in a large dataset would be very computationally intensive and inapplicable in practice. 

To be able to query images, a representation of images needs to be computed such that some evaluation of similarity can be applied. All current methods use the notion of detecting salient regions of an image, \emph{features}, and describing them by some vector, a \emph{descriptor}, then using the distance between feature descriptors as a measure of similarity. Basic feature detectors could be corner detectors (such as the Harris detector) and a descriptor could simply be a vector of the local grayscale values, however a method like this is prone to variance due to changes in viewpoints and lighting. Many alternate methods have been developed, for example, work by D. Lowe \cite{lowe1999} shows that using a detector and descriptor that is invariant to many of the distortions that occur when taking different images of the same object provides a good representation for matching.

The standard method for object matching is to return a list of the most similar images in the dataset to the query image, ranked by similarity, and determine the contained object in the query image from the labels of those in the list. Web search engines encounter a similar problem - for a query string, the most similar web pages are returned in a list. It is no surprise that the standard methods for text retrieval \cite{searchengines2003,googlearchitecture2003} are used as a basis for image retrieval. By using a bag of words model, applying weightings to words, and employing inverted file structures, an initial filtering is performed that prevents the need for examination of every document in the corpus.

This mimicking of text retrieval systems is shown in recent work \cite{recognitiontree2006,videogoogle2003}, using ``visual words''\footnote{The term ``words'' in the context of image retrieval will refer to this notion of ``visual words''.} contained within images as an analogy to words and phrases within text documents. Visual words are a quantised form of feature descriptors. Despite their apparent similarities, the noisiness and abundance of visual words in images makes the problem of image retrieval rather different. In text retrieval, the words in the query are generally inputted by a user with the view to producing a relevant set of results, whereas in the image retrieval case, the words in the query are a product of the imaging system and will contain many distractor words not relevant to the desired query result. This means that while in text retrieval a good document in the results will contain all the query words, in image retrieval a good image in the results may contain only 1\% of the query words (e.g. 30 verified matched words from 3000 query visual words).

Another reason for this large difference to text retrieval is that there is spatial information associated with visual words that is of use to the query and in determining matches. In text retrieval, the user may not be interested in preserving the exact order of the search words and their positions in documents may not be of any significance. A visual query however must take into account the spatial configuration of the words, as an object is in essence defined not only by the visual words it exhibits, but the positions and scale of those visual words within an object. 

The main advancements in recent years to create state of the art systems \cite{philbin2007} have been made by improving visual vocabulary and incorporating the spatial information. Early systems \cite{videogoogle2003} used a flat k-means clustering on feature descriptors to create the visual words, but this is difficult to scale. Cluster hierarchies \cite{hierarchies2006} allow the size of vocabularies to be greatly increased, and using approximate nearest neighbours and a random forest algorithm \cite{randomforest2005}, far superior performance using very large vocabularies (100K words and greater) can be achieved. For incorporating spatial information, re-ranking of the matched images is performed to take into account the spatial correspondences of the visual words. Two views of the same rigid object are related by a homography, and the estimation of correspondences due to this type of mapping is well discussed in \cite{zisserman2004}. The transformation is used to re-rank the matched images based on their spatial information \cite{philbin2007, videogoogle2003}. 

The state of the art as described in \cite{philbin2007} is used as the basis for this project.
...
This process creates a structured dataset of model images from the Wikipedia pages visited. For the List of Structures in London dataset used, there are 732 classes which had 3963 images associated with them (on average 6 images per class).
...
While database-side feature augmentation is not suited to this application, the overall idea of feature augmentation can still be used. Rather than database-side feature expansion, an independent source of images is used for augmentation. The turbo-boosting can be thought of as expanding the information content of a ground-truth image database using an external dataset.

The external source of images used for turbo-boosting in this project is Microsoft's Bing image search, though any labelled image dataset can be used. The rationale for using a search engine is that labelled images for each class (a class being an object that could be contained with in an image) can be generated by issuing a search query. Therefore, for images of a class, the candidate images to use for augmentation are the results of a search query of the class name. The result is that turbo-boosting keeps the accuracy of the ground truth of your dataset but expands the information content of the dataset images.

Listing~\ref{lst:turboboost} shows the high level MATLAB code that outlines the implementation of turbo-boosting. A class centric approach is taken, so the images from each class are augmented in the same loop. For each class, the candidate images for augmentation are downloaded, called augmentation images or \emph{turbo images}. Section~\ref{sec:turboimages} describes the data gathering process in detail. Then, for each model image belonging to the class in question, each downloaded turbo image is tested to see if it can be spatially verified against the model image. If a turbo image is spatially verified, a rectangle that is the bounding box of the spatially verified visual words is computed (with a margin of tolerance). All the visual words contained within the rectangle on the turbo image are added to the visual words of the model image at the position mapped by the affine transformation relating the two images. 

\linespread{1} % single line spacing
\lstset{language=Matlab,caption=High level MATLAB code to outline the augmentation stage of the turbo-boosting process.,label=lst:turboboost}
\begin{lstlisting}[frame=single]
%% MATLAB
% Get a list of classes
classes = get_class_names(db);   % db is the database of model images
for i=1:length(classes)
    ...
    for j=1:length(model_images)
        ...
        for k=1:length(turbo_images)
            ...
            verified, verified_model_words, verified_turbo_words, affine_transformation = spatially_verify(correspondences);
            if verified
                % Get the region within the turbo_image that contains
                % words
                bounding_rectangle = get_bounding_box(verified_turbo_words);
                verified_turbo_words = words_within_rectangle(bounding_rectange, verified_turbo_words);
                % Map the verified turbo words within the bounding
                % rectangle onto the model image and augment these words
                model_im_words = augment_words(model_im_words, verified_turbo_words, affine_transformation);
            end
        end
...
\begin{figure}[htb]
\centering 
\subfloat[]{\label{fig:turbodownload}\includegraphics[width=0.9\textwidth]{images/turboboosting-001.png}}
\\
\subfloat[]{\label{fig:turboaugment}\includegraphics[width=0.9\textwidth]{images/turboboosting-002.png}}
\caption{The turboboosting process [I NEED TO MAKE THIS MUCH BETTER].}
...
Further work would aim to increase the yield and scale the size of the database. Better filtering of the Wikipedia dataset could be done, as well as the removal of distracting features that do not uniquely define a class \cite{turcot09}. The labelling of objects could also be done in a more probabilistic way - instead of simply defining the object found by the class of the first image that is spatially verified, use some algorithm to determine the most likely object given the top spatially verified images, with some uncertainty score.


[APPENDIX ON VALIDATION PROCESS??]
