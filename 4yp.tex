...
The system builds upon the state-of-the art in visual object retrieval from large databases -- that is allowing objects to be recognised by searching against millions of images. The images contained within the articles of Wikipedia \cite{wikipedia} are used for the dataset of images, with each article defining an object. In this project, a subset of Wikipedia articles is used to build the object database: the articles linked to by the ``List of Structures in London'' \cite{listofstructures} Wikipedia page. The resulting system can automatically recognise and annotate multiple objects within a query image. 

Various changes are implemented to increase the performance of the baseline system. Geometric improvements to the spatial verification stage as well as changes to the image feature detector and descriptor methods are shown to increase recognition performance. An additional novel process was added which augments the database of object images with the information content of an external source of labelled images. This improvement is shown to drastically increases performance.
...
The web contains billions of images, and they are often the focus of attention on the web pages that include them. However, there is almost no information about the content of these images. Images on websites are purely binary data files, occasionally with some associated metadata included in their containing HTML\footnote{Hyper Text Markup Language \url{http://en.wikipedia.org/wiki/HTML}} code. Very little information is available about the images, let alone the objects or scenes contained within them. The aim of this project is to create a system which automatically recognises the objects within images, thus releasing the information within them. This is a large scale object retrieval problem.
...
The report starts by introducing the relevant literature in Chapter~\ref{chpt:background}. This gives a holistic view of the recent innovations in large scale object recognition and retrieval and provides the basis of knowledge used in the project.
...
\item Test images gathered from Google Images, described in Section~\ref{sec:validationimages} and used in Subsection~\ref{sub:performance}.
\end{itemize}
...
With this measure of performance, the performance of the baseline system (Chapter~\ref{chpt:system}) can be quantified, and the effects of the improvements described in Chapter~\ref{chpt:geoimprovements} and Chapter~\ref{chpt:turboboosting} can interpreted.
...
\label{chpt:system}
The baseline system draws upon the research and literature mentioned in Chapter~\ref{chpt:background}. Upon starting the project, an application was provided written in MATLAB and C. However it simply ranks the similarity of database images to a query image, and the database is one of structures in Oxford that has been well researched. There was therefore a very large amount of work to take the starter application forward to the full system proposed by this project.
...
\begin{figure}[h!tb]
\centering 
\subfloat[Big Ben image 1]{\includegraphics[width=0.18\textwidth]{images/bigben.jpg}}
~
\subfloat[Big Ben image 2]{\includegraphics[width=0.3\textwidth]{images/bigben2.jpeg}}
\\
\subfloat[Matches based purely on SIFT feature similarity]{\includegraphics[width=0.4\textwidth]{images/spatial_verification1.pdf}}
\subfloat[Matches after RANSAC spatial verification]{\includegraphics[width=0.4\textwidth]{images/spatial_verification2.pdf}}
...
\begin{figure}[h!tb]
\centering 
\subfloat[NOSAC]{\includegraphics[width=0.65\textwidth]{images/nosacmatch2.pdf}}
\\
\subfloat[Affine Invariant]{\includegraphics[width=0.65\textwidth]{images/affinematch.pdf}}
...
\end{tabular}
\end{center}
\caption{The yield achieved for each improvement method with different thresholds for the number of RANSAC inliers required for a match.}
...
This chapter describes the improvements made using a technique to inflate the visual word content of the model images in the database. This novel technique, termed \emph{turbo-boosting}, increases the yield performance of the system.
...
Turbo-boosting is the spatial augmentation of features from images of a completely independent dataset. The process is exactly that of spatial database-side feature augmentation \cite{rootsift12}, however rather than the model images being used for augmentation, labelled images from Microsoft's Bing image search are used. A number of images, called \emph{turbo images} are downloaded for each class, by issuing a query to the Bing search engine (see Section~\ref{sec:turboimages}). For example, the turbo images for the class \lstinline!Tower_Bridge! are shown in Figure~\ref{fig:turboturboimages}. For each model image, if a turbo image matches it, the visual words of the turbo image are projected onto the model image.


Take, for example, the image in Figure~\ref{fig:turbomodelimage} as the first model image to be turbo-boosted. Each turbo image is tested to see if it matches the model image. In this example, the turbo image in Figure~\ref{fig:turboturboimage} matches against the model image, with the match being shown in Figure~\ref{fig:turbomatch}. The matched region is the bounding rectangle of the spatially verified visual words, with a slight margin. \emph{All} of the visual words contained with the matched region of the turbo image are projected onto the model image, as shown in Figure~\ref{fig:turboaugment}. The ellipse representing each of the turbo image's words within the matched region are projected onto the model image using the affine transformation relating the scenes of the two images that is computed from the matched words. The model image has now been spatially augmented with the visual words of the turbo image.
...
Figure~\ref{fig:turboresult} shows a case where the query image, Figure~\ref{fig:turboresultqueryimage}, cannot be matched to any model images. The inlier threshold is set to 5 so the model image, Figure~\ref{fig:turboresultmodelimage}, does not match with the query image as there are only 4 inliers (Figure~\ref{fig:turboresultvanilla}). However, the model image  can be turbo-boosted by the turbo image in Figure~\ref{fig:turboresultturboimage} due to the match made in Figure~\ref{fig:turboresultturbomatch}, leading to the augmentation of the words in the matched region of the turbo image. The turbo-boosted model image (Figure~\ref{fig:turboresultaugmentedmodel}) now matches against the query, as shown in Figure~\ref{fig:turboresultturbo}. There are many examples of cases similar to this one -- where the model image is perfectly representative of the object, but does not have enough stable visual words to match reliably. However, due to the large number of turbo images, some augmentation occurs which pulls in better stable visual words.
...
\begin{figure}[t!]
\centering 
\subfloat[Query image with words]{\label{fig:turboresultqueryimage}\includegraphics[width=0.25\textwidth]{images/query_words.pdf}}
~
\subfloat[Model image with words]{\label{fig:turboresultmodelimage}\includegraphics[width=0.22\textwidth]{images/model_words.pdf}}
~
\subfloat[Turbo image with words]{\label{fig:turboresultturboimage}\includegraphics[width=0.25\textwidth]{images/turbo_words.pdf}}
\\
\subfloat[The match before turbo-boosting]{\label{fig:turboresultvanilla}\includegraphics[width=0.45\textwidth]{images/query_vanillamodel.pdf}}
~
\subfloat[The turbo image matching the model image]{\label{fig:turboresultturbomatch}\includegraphics[width=0.45\textwidth]{images/model_turbo.pdf}}
\\
\subfloat[The model image's words after turbo-boosting]{\label{fig:turboresultaugmentedmodel}\includegraphics[width=0.22\textwidth]{images/turbo_model_words.pdf}}
~
\subfloat[The match after turbo-boosting]{\label{fig:turboresultturbo}\includegraphics[width=0.5\textwidth]{images/query_turbomodel.pdf}}

\caption{The query image Figure~\ref{fig:turboresultqueryimage} will not be identified unless model image Figure~\ref{fig:turboresultmodelimage} is augmented with the words of Figure~\ref{fig:turboresultturboimage}. Due to the match in Figure~\ref{fig:turboresultturbomatch}, the positions of the visual words in the turbo image are projected onto the model image, Figure~\ref{fig:turboresultaugmentedmodel}.}
...
The resulting interface is a MATLAB function \lstinline!get_objects()! that takes a single argument of the disk location of the query image to recognise the objects in. The output is a structure of the object classes found and their bounding rectangles in the query image. The function takes around five seconds to run.
...
Using the distributed system for pre-computation and testing results in a far quicker process, reducing the time from 15-20 hours for some jobs to just one hour.
...
Upon clicking the ``Tag Image'' button, the real-time log is informs the user of the progress of the matching process (Figure~\ref{fig:log}). The real-time log is achieved by the MATLAB process writing a log file and the Javascript app periodically asking the web server for the contents of the latest version of that file.
...
while matlab.running:
	# Matlab is already processing something, wait
        time.sleep(2)
resp = matlab.run('~/Sites/4YP/visualindex/wikilist_dataset/demo_wiki_get_objects.m', {'image_path': query_image_path, 'display': 1, 'log_file': 'logs/123-log.txt'}, maxtime=999999)
# Get the results
result = resp['result']
\end{lstlisting}
\linespread{2} % double line spacing

With this bridge, running any MATLAB function from Python becomes trivial and so the web application can run the MATLAB matching process.
...
\caption{A selection of annotated images. Clockwise from top left: London Central Mosque, Abbey Mills Pumping Station, Bloomsbury Square, University of Greenwich, Victoria and Albert Museum, and St James's Church, Piccadilly.}
...
\begin{thebibliography}{99}
	...
	\bibitem{listofstructurespdf}
		\url{http://www.2shared.com/document/KQ8GqF7Z/List_of_structures_in_London.html}
...
retrieval with large vocabularies and fast spatial matching. 
	...
	\bibitem{randomforest1}
		...
		In \emph{Neural Computing, 9(7):1545Ð1588}, 1997.
