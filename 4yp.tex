...
Various changes are implemented to increase the performance of the baseline system. Geometric improvements to the spatial verification stage as well as changes to the descriptor and detector methods for image features are shown to increase recognition performance. An additional novel process was added which augments the database of object images with the information content of an external source of labelled images. This improvement is shown to drastically increases performance.
...
Figure~\ref{fig:turboresult} shows a case where the query image, Figure~\ref{fig:turboresultqueryimage}, cannot be matched to any model images. The inlier threshold is set to 5 so the model image, Figure~\ref{fig:turboresultmodelimage}, does not match with the query image as there are only 4 inliers (Figure~\ref{fig:turboresultvanilla}). However, the model image  can be turbo-boosted by the turbo image in Figure~\ref{fig:turboresultturboimage} due to the match made in Figure~\ref{fig:turboresultturbomatch}, leading to the augmentation of the words in the matched region of the turbo image. The turbo-boosted model image now matches against the query, as shown in Figure~\ref{fig:turboresultturbo}. There are many examples of cases similar to this one -- where the model image is perfectly representative of the object, but does not have enough stable visual words to match reliably. However, due to the large number of turbo images, some augmentation occurs which pulls in better stable visual words.
