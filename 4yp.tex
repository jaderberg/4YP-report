...
This chapter describes a technique to inflate the visual word content of the model images in the database. This novel technique, defined as \emph{turbo-boosting}, increases the performance by xyz as shown in Table HAHAHAH.
...
The external source of images used for turbo-boosting in this project is Microsoft's Bing image search, though any labelled image dataset can be used. The rationale for using a search engine is that labelled images for each class (a class being an object that could be contained with in an image) can be generated by issuing a search query. Therefore, for images of a class, the candidate images to use for augmentation are the results of a search query of the class name. More detail of gathering images for potential augmentation (turbo-boosting images) is given in Section~\ref{sec:turboimages}.
...
Turbo-boosting uses the ground truth of your dataset to expand the information content of your images.
THIS IS A GENERAL STRATEGY FOR INCREASING MATCHING ON ANY!!! DATASET
