...
While database-side feature augmentation is not suited to this application, the overall idea of feature augmentation can still be used. Rather than the model images dataset being used for feature expansion, an independent source of images is used for augmentation. Turbo-boosting can be thought of as expanding the information content of a ground-truth image database using an external dataset. Each model image is turbo-boosted in turn, outputting a new, more informative model image, which is used in place of the original to create a turbo-boosted dataset.
